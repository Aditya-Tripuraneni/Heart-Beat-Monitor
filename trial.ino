...
void loop() {
  int heartBPM = pulseSensor.getBeatsPerMinute(); // heart beats per minute 
  lcd.setCursor(0,0);  // text displayed starting from left side 
  measureHeartBeat(heartBPM); // method to measure our heart beat

  if (heartBPM == 0){ // if we don't detect any heart beat, let user no nothing is detected
    lcd.print("No heart beat detected.");
    }

  delay(5); // used to delay to allow for easier processing
}
